...
#Отображение списка записей дневника 
@login_required
def articles(request): 
    articles = Article.objects.filter(author=request.user)
    return render(request, 'web_app/article_list.html', {'articles' : articles})

#Отображение списка записей Плана
@login_required
def plans(request): 
    plans = Plan.objects.filter(author=request.user)
    return render(request, 'web_app/plans_list.html', {'plans' : plans})

def lists(request):
    logger.info('Lists page accessed')
    return render(request, 'web_app/lists.html')
